<?php
	...
	if($_SERVER["REQUEST_METHOD"] == "POST") {
		//get form fields, remove white space
		$name = strip_tags(trim($_POST['name']));
		$email = filter_var(trim($_POST['email']), FILTER_SANITIZE_EMAIL);
		$phone = strip_tags(trim($_POST['phone']));
		$year = trim($_POST['year']);
		$interests = trim($_POST['interests']);
		$dentalxp = trim($_POST['dentalxp']);
		$story = trim($_POST['story']);
		
		if(empty($name) OR empty($phone) OR empty($year) OR empty($interests) OR empty($dentalxp) OR empty($story) OR !filter_var($email, FILTER_VALIDATE_EMAIL)) {
			//set 400 response code,exit
			http_response_code(400);
			echo "OOPS. There was a problem with your submission. Please complete the form again.";
			exit;
		}
		...
		//set email subject
		$subject = "New Project Smile Applicant";
		...
		$body .= "Dental Experience: " . $dentalxp . "\n";
		$body .= "Story: " . $story . "\n";
		...
		<div class="section grey lighten-4">
			<div class="row container">
				<div class="col s12">
					...
					<p>Thank you for applying to join Project Smile. Your
